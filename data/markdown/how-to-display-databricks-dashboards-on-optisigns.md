...
### In this article, we'll walk you through setting up Databricks to display on your OptiSigns digital signs.
  ...
  * Create a Databricks App in OptiSigns
...
These values correspond to the In DataBricks section of this article.
...
**NOTE**  
---  
One connection is equal to one Databricks workspace.  
  
If you have more than one workspace, add a separate connection for each.  
...
**Only needed if you published with** _**Individual data permission**_. With _Share data permission_ , queries run on the publisher's credentials and you can skip this step.
...
Tick **USE SCHEMA**(under _Prerequisite_) and **SELECT**(under _Read_). Leave everything else unticked. The screens only need to read.
...
That covers every schema in the catalog, including ones created later, so there's nothing to repeat. The trade-off is that the service principal can then read _all_ the data in that catalog, not just what the dashboards display.
