...
![Choosing Application permissions, then searching for and ticking TeamworkDevice.Read.All](https://support.optisigns.com/hc/article_attachments/54863299150099)
  ...
  3. Tick its checkbox. The description reads _Read Teams devices_ , and **Admin consent required** says **Yes** — that matters in the next step.
...
The permission has been _requested_ , not _approved_. In this state Microsoft still issues OptiSigns an access token — it simply carries no privileges, so every attempt to read your rooms returns **403 Forbidden**. Nothing about the app registration looks broken; it just doesn't work.
...
Copy the **Value** — the long string in the _Value_ column. **Not** the _Secret ID_ beside it. The two sit side by side and look equally cryptic, and pasting the Secret ID is the second most common mistake here.
...
Read that last part — `Roles on the request ''` — as: _the token is valid and it carries no permissions._ Your client ID, tenant ID and secret are all fine. Either admin consent is missing, or the permission was added under **Delegated** instead of **Application**. Fix it in three steps, and do all three:
